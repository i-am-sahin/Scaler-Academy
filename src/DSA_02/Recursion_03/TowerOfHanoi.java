...
public class TowerOfHanoi {
    ...
    public ArrayList<ArrayList<Integer>> towerOfHanoi(int A) {
        // source(a) -> n disk are kept on source
        //Helper poll(b) - >
        //Destination Poll(c) ->
    ...
    public void solve(int disk,int source,int destination){
        if(disk == 1) {
        ...
        // ek disk koo source main chor ker n-1 disk koo helper main dal dungea
        solve(disk - 1,source, 6 - source - destination);


        //ek joo disk bacha hua tha source main usko Destination main daii dungea
        ArrayList<Integer> move = new ArrayList<>();
        move.add(disk);
        move.add(source);
        move.add(destination);
        ans.add(new ArrayList<>(move));
        // n-1 disk abb helper sea Destination main dal dungea
        solve(disk - 1, 6 - source - destination,destination);
    }
    //1 + 2 + 3 = 6;
}
